...
func _init(total_money: int, total_ether: int, breakdown: Dictionary) -> void:
	...
	for ore_id: String in breakdown:
		...
		var name_label := UIKit.label("%s x%d" % [ore["name"], count], 18)
		name_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		row.add_child(name_label)
		row.add_child(UIKit.label("%d $" % (int(ore["value"]) * count), 18, UIKit.GOOD))
		content.add_child(row)
	var total := UIKit.label("Total: %d $" % total_money, 28, UIKit.GOOD)
	total.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	content.add_child(total)
	if total_ether > 0:
		var ether_label := UIKit.label("+%d ether" % total_ether, 20, UIKit.ETHER)
		ether_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		content.add_child(ether_label)
	var btn := UIKit.action_button("NICE!")
